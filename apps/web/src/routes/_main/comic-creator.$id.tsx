...
export const Route = createFileRoute("/_main/comic-creator/$id")({
  component: RouteComponent,
  loader: async ({ params }) => {
    const data = await queryClient
      .ensureQueryData(getComicCreatorQueryOptions(params.id))
      .catch((error: unknown) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      {
        title: `NeXusTC - ${loaderData?.creator.name ?? "Creador"}`,
      },
    ],
  }),
});
