...
export type HomeData = {
  featuredPosts: LoaderResult<
    Awaited<ReturnType<typeof orpcClient.post.getFeatured>>
  >;
  recentUsers: LoaderResult<
    Awaited<ReturnType<typeof orpcClient.user.getRecentUsers>>
  >;
  weeklyGames: LoaderResult<
    Awaited<ReturnType<typeof orpcClient.post.getWeekly>>
  >;
};
...
function useHomeData() {
  const data = use(HomeDataContext);
  if (!data) {
    throw new Error("Home data is unavailable");
  }
  return data;
}

export function HomeClient({ loaderData }: { loaderData: HomeData }) {
  return (
    <HomeDataContext value={loaderData}>
      <HomeContent />
    </HomeDataContext>
  );
}
...
function HeroSection() {
  const { featuredPosts } = useHomeData();
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 min-h-0 md:max-h-150">
      ...
      {secondary.length > 0 && (
        <div className="grid grid-cols-2 gap-4 md:aspect-1/2 md:w-1/3 md:shrink-0 md:grid-cols-none md:grid-rows-2">
          {secondary.map((post) => (
      ...
      <div className="flex flex-wrap gap-1.5">
        {tags.map((tag) => (
          <Link
            className="flex grow"
            key={tag.id}
            href={{ pathname: "/juegos", query: { tag: tag.id } }}
            prefetch={false}
          >
