...
type WebhookRequests = Awaited<
  ReturnType<typeof orpcClient.patreon.admin.listWebhookRequests>
>;

export function WebhooksClient({
  initialData,
}: {
  initialData: WebhookRequests;
}) {
  ...
  const webhookRequests = webhookRequestsQuery.data;

  const requests = webhookRequests?.requests ?? [];
  const total = webhookRequests?.total ?? 0;
