...
export function AgeVerificationDialog() {
  ...
  const updateDisclaimerReadState = useCallback(() => {
    ...
    const scrollRemaining =
      disclaimer.scrollHeight - disclaimer.scrollTop - disclaimer.clientHeight;
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(updateDisclaimerReadState);
    resizeObserver.observe(disclaimer);
    window.addEventListener("resize", updateDisclaimerReadState);

    return () => {
      cancelAnimationFrame(animationFrame);
      resizeObserver.disconnect();
      window.removeEventListener("resize", updateDisclaimerReadState);
    };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="max-h-[95dvh] data-open:fade-in-0 data-open:zoom-in-95 data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 flex w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-border/70 bg-card/90 shadow-2xl outline-none backdrop-blur-xl duration-200 data-closed:animate-out data-open:animate-in sm:max-w-130">
          <div
            ...
            onScroll={updateDisclaimerReadState}
            ref={disclaimerRef}
          >
            ...
            <header className="relative flex flex-col items-center gap-4 px-8 pt-10 pb-4 text-center">
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                >
                  ...
                  temáticas sexuales. Necesitamos confirmar que eres mayor de
                  edad antes de continuar.
                </Dialog.Description>
              </div>
            </header>
          ...
          <div className="relative flex flex-col gap-2 border-border/50 border-t bg-background/30 p-4 sm:flex-row-reverse sm:items-center sm:gap-3">
            <div className="flex w-full flex-col gap-1.5 sm:flex-1">
              <Button
                className="group h-11 w-full gap-2 rounded-lg font-semibold text-[13.5px] tracking-wide shadow-glow-primary/25 transition-[background-color,box-shadow] duration-200 hover:shadow-glow-primary/45"
              ...
              {hasReadDisclaimer ? null : (
                <p className="text-center text-[11px] text-muted-foreground leading-snug">
                  Desplázate hasta el final del aviso para continuar.
                </p>
              )}
            </div>
            <Button
